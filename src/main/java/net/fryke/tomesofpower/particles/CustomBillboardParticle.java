...
/**
 * A {@link CustomAbstractParticle} which renders a camera-facing sprite with a target texture scale.
 */
@Environment(value=EnvType.CLIENT)
public abstract class CustomBillboardParticle extends CustomAbstractParticle {
    protected float scale;

    protected CustomBillboardParticle(ClientWorld clientWorld, double d, double e, double f) {
        super(clientWorld, d, e, f);
        this.scale = 0.1f * (this.random.nextFloat() * 0.5f + 0.5f) * 2.0f;
    }

    protected CustomBillboardParticle(ClientWorld clientWorld, double d, double e, double f, double g, double h, double i) {
