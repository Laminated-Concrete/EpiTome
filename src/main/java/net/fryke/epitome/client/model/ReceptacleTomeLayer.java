...
public class ReceptacleTomeLayer extends GeoRenderLayer<ReceptacleBlockEntity> {
    private final ReceptacleTomeLayerRenderer receptacleTomeLayerRenderer = new ReceptacleTomeLayerRenderer("tome");
    private final ReceptacleTomeLayerRenderer receptacleKnowledgeTomeLayerRenderer = new ReceptacleTomeLayerRenderer("knowledge_tome");

    public ReceptacleTomeLayer() {
        super(new ReceptacleTomeLayerRenderer("tome"));
    }
    ...
    public void render(MatrixStack poseStack, ReceptacleBlockEntity animatable, BakedGeoModel bakedModel, RenderLayer renderType, VertexConsumerProvider bufferSource, VertexConsumer buffer, float partialTick, int packedLight, int packedOverlay) {
        ...
        if(tomeIdentifier != null && tomeIdentifier.getNamespace().equals("epitome") && !tomeIdentifier.getPath().equals("null")) {
            ...
            if(possibleTome instanceof KnowledgeTomeItem tomeItem) {
                Identifier textureId = receptacleKnowledgeTomeLayerRenderer.getTextureLocation(tomeItem);
                RenderLayer receptacleTomeLayerRenderType = RenderLayer.getEntityCutoutNoCull(textureId);
                BakedGeoModel actualBakedModel = receptacleKnowledgeTomeLayerRenderer.getGeoModel().getBakedModel(receptacleKnowledgeTomeLayerRenderer.getGeoModel().getModelResource(tomeItem));

                receptacleKnowledgeTomeLayerRenderer.actuallyRender(poseStack, tomeItem, actualBakedModel, receptacleTomeLayerRenderType, bufferSource, bufferSource.getBuffer(receptacleTomeLayerRenderType),
                    false, partialTick, packedLight, packedOverlay, 1, 1, 1, 1);
            }

            if(possibleTome instanceof TomeItem tomeItem) {
